...
export class NotificationUpdatePage {
  ...
  locationCodeInput = element(by.id('field_locationCode'));
  ...
  async setLocationCodeInput(locationCode: string): Promise<void> {
    await this.locationCodeInput.sendKeys(locationCode);
  }

  async getLocationCodeInput(): Promise<string> {
    return await this.locationCodeInput.getAttribute('value');
  }
